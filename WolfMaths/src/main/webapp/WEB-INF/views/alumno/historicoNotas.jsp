...
<div class="contTable">
	<table class="table well">
		...
		<tbody>
				...
				<c:forEach var="correccion"
					items="${tareasAlumnoForm.listaHistorico}"
					varStatus="rowCounter">
					<c:set var="trStyle" value="active"/>
					<c:if test="${not empty correccion.nota}">
						<fmt:parseNumber var="notaValue" type="number" value="${fn:replace(correccion.nota,'.',',')}" />
					...
					<tr class="${trStyle}" id="correccion_Alumno_${correccion.id}">

						<td>
							<p style="white-space: pre-wrap;">${rowCounter.index+1}</p>
						</td>
						<td>
							<p style="white-space: pre-wrap;">${correccion.nota}</p>
						</td>
						<td>
							<p style="white-space: pre-wrap;">${correccion.comentario}<p>
						</td>
					</tr>
				</c:forEach>
		</tbody>
